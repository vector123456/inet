...
class INET_API CPacketDropperFunction : public cObject, public IPacketDropperFunction
{
  ...
  public:
    ...
    virtual void dropPackets(IPacketCollection *collection) const { return packetDropperFunction(collection); }
    virtual PacketDropperFunction getDropperFunction() const { return packetDropperFunction; }
};

#define Register_Packet_Dropper_Function(name, function) \
    class INET_API name : public ::inet::queue::CPacketDropperFunction { public: name() : CPacketDropperFunction(function) { } }; \
    Register_Class(name)
